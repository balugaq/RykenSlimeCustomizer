...
public class RecipeMachineReader extends YamlReader<AdvancedCustomMachine> {
    @Override
    public String getFileName() {
        return Constants.RECIPE_MACHINES_FILE;
    }

    public RecipeMachineReader(File file, ProjectAddon addon) {
        super(file, addon);
    }
    ...
    public AdvancedCustomMachine readEach(String s) {
        ...
        if (capacity < 0) {
            Debug.error(file, section, "缺少或配置错误 '能源容量' (capacity)", 0, Integer.MAX_VALUE);
            return null;
        }

        int energy = section.getInt("energyPerCraft", 0);
        if (energy < 0) {
            Debug.error(file, section, "缺少或配置错误 '能量消耗' (energyPerCraft)", 0, Integer.MAX_VALUE);
            return null;
        }
        ...
        var ticker = MachineTicker.create(file, machine, section, menu, addon, MachineTicker.Type.RECIPE);
