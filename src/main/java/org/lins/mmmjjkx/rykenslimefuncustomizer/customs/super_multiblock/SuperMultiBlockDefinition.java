...
public class SuperMultiBlockDefinition extends SuperMultiBlockManager {
    ...
    public SuperMultiBlockDefinition(Map<String, MultiBlockPart> mapping, Map<Vector3i, MultiBlockPart> map, HorizonDirection baseDirection) {
        this.mapping = Map.copyOf(mapping);
        this.map = Map.copyOf(getRotatedMap(map, baseDirection));
    }

    public int count(String mappingName) {
    ...
    @Nullable
    public Vector findFirstValue(HorizonDirection direction, String mappingName) {
        var part = getPart(mappingName);
    ...
    public static Map<Vector3i, MultiBlockPart> getRotatedMap(Map<Vector3i, MultiBlockPart> map, HorizonDirection direction) {
    ...
    public Map<Vector3i, MultiBlockPart> getMap(HorizonDirection direction) {
        if (rotatedMap.containsKey(direction)) return rotatedMap.get(direction);
        var rotated = getRotatedMap(map, direction);
        rotatedMap.put(direction, rotated);
        return rotated;
    }

    
    public Set<Location> getLocations(Location coreLocation, HorizonDirection direction) {
